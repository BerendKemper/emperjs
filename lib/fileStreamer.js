...
class MultiReadableFile {
    ...
    constructor(filepath, streamer) {
        ...
        process.nextTick(this.#accessReadableFile, this);
    }
    #accessReadableFile(file) {
        const req = new FSReqCallback()
        req.oncomplete = file.#accessReadableFileAfterOpen;
        req.file = file;
        open(toNamespacedPath(file.filepath), O_RDONLY, 0o666, req);
    }
    #accessReadableFileAfterOpen(error, fd) {
        ...
        file.fd = fd;
        const req = new FSReqCallback();
        req.oncomplete = file.#accessReadableFileAfterStat;
        req.file = file;
        fstat(file.fd, false, req);
    }
    #accessReadableFileAfterStat(error, stats) {
        const file = this.file;
    ...
    newContext(streamer) {
        ...
        process.nextTick(this.#readChunkNewContext, this);
    }
    #readChunkNewContext(file) {
...
class MultiStreamableFile extends CallbackQueue {
    ...
    send(filepath, end) {
        ...
        return this.push(this.#queueSendFile, { filepath, end }).response;
    }
    #queueSendFile(next, { filepath, end } = context) {
        this.mustEnd = end;
        this.next = next;
        streaming[filepath]
        ...
        this.#bytes += bytes;
        if (this.mustEnd)
            return this.push(this.#destroy, null).response.end(buffer, "utf8", () => process.nextTick(this.next));
        this.response.write(buffer, "utf8", () => process.nextTick(this.next));
    }
    #destroy() {
        this.response.report(this.#bytes);
        this.response = null;
        this.next = null;
        this.destroy();
    }
