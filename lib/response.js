...
module.exports = emper => {
    ...
    return class Response extends require("http").ServerResponse {
        ...
        sendFile(filepath) {
            return (this.#fileStreamer ??= new FileStream(this, this.#queue)).send(filepath);
        }
        #onClose() {
            if (logger)
        ...
        static {
            emper.handleEndpoint = (request, response, route) => {
                for (var path of route)
                    for (const middleware of path.middleware)
                        response.#queue.push(middleware);
                const apiEndpoint = path.endpoint[request.method];
