...
class Request extends require("http").IncomingMessage {
	...
	onEnd() {
		...
		if (this.body.length > 0) {
			...
			return parser ? parser.parse(Buffer.concat(this.body).toString(), this) : this.socket._httpMessage.sendError(400, new Error(`Failed to identify content-type: ${this.headers["content-type"]}`));
		}
		this.onParsedBody({});
	};
	onParsedBody(parsed) {
