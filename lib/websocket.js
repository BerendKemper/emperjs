...
module.exports = emper => {
    ...
    function dataListener(data) {
        ...
        for (let i = 0; i < data.byteLength; i++) {
            const byte = data[i];
            const atMin4 = data[i - 4];
            // if (atMin4 === byte) {
            if (atMin4 === byte && !(byte === 0 && data[i - 1] === 0)) {
                if (pattern[patternIx] === data[i - 1]) {
                    occurences[patternIx]++;
                    if (++patternIx - patternStart === 4)
                        patternIx = patternStart;
                } else {
                    patternStart = patternIx = pattern.indexOf(byte, patternStart);
                }
            } else {
                occurences[pattern.push(byte) - 1] = 1;
                // patternStart = pattern.push(byte) - 1;
                // occurences[patternStart] = 1;
            }
        }
        var print = [];
        for (const ix in pattern) {
            print.push(occurences[ix] > 1
                ? `${pattern[ix]}(${occurences[ix]})`
                : String(pattern[ix]));
        }
        console.log("byteLength:", data.byteLength);
        console.log("pattern: [ " + print.join(", ") + " ]");
        console.log("")
