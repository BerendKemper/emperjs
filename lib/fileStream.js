...
class ErrorOnOpen extends Error {
    constructor(error) {
        const ixMainPath = error.stack.indexOf(mainPath);
        return ixMainPath !== -1
            ? super(`${error.stack.slice(0, ixMainPath)}.${error.path.slice(mainPath.length).split("\\").join("/")}'`)
            : error;
    }
}
...
class MultiReadableFile {
    ...
    #constructAfterOpen(error, fd) {
        ...
        if (file.error ??= error && new ErrorOnOpen(error)) {
