...
class Routes {
    ...
    addEndpoint(path, method, callback) {
        ...
        const route = this.route.add(path.split("/").slice(1, path.at(-1) === "/" ? -1 : Infinity));
        if (route.endpoint[method])
            throw new Error(`There is already an API at ${method} ${path}`);
        return route.endpoint[method] = callback;
    }
    addMiddleware(path, middleware) {
        this.route.add(path.split("/").slice(1, path.at(-1) === "/" ? -1 : Infinity)).middleware.push(middleware);
    }
