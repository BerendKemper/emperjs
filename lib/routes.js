...
class Route {
    ...
    add(type, destination) {
        ...
        if (sign[0] === ":") {
            ...
            const route = new Route(this.distance + 1, sign);
            this.relative.push(route);
            return route[type](destination);
        } else if (!this.absolute[sign])
            this.absolute[sign] = new Route(this.distance + 1, sign);
    ...
    proceed(request, context) {
        ...
        if (this.distance < context.targets.length) {
            const sign = context.targets[this.distance];
            if (this.absolute[sign])
                return this.absolute[sign].proceed(request, context);
            for (const route of this.relative) {
                route.proceed(request, context);
                if (context.found)
...
class Routes {
    ...
    findEndpoint(request) {
        ...
        const context = {
            fault: null,
            faultyRoute: noFaultyRoute,
            found: false,
            route: [],
            targets: urlSplit.at(-1) === ""
                ? urlSplit.slice(1, -1)
                : urlSplit.slice(1)
        };
        this.route.proceed(request, context);
        return context;
    }
}
