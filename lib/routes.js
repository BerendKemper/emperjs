...
class Route {
    ...
    add(destination) {
        ...
        if (httpMethods.has(sign)) throw new Error("Using CRUD operations in the path is forbidden");
...
class Routes {
    ...
    add(path, method, callback) {
        ...
        const route = this.route.add(destination);
    ...
    walk(request) {
        ...
        if (api) {
            ...
            request.socket._httpMessage.sendError(400, new Error(`Could not identify "/${failData.fault}" in "${request.urlPath}"`));
        else
