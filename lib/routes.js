"use strict";
const CRUD_opertions = { GET: true, POST: true, PUT: true, DELETE: true };
class Route {
	#absolute = {};
	#relative = [];
	#endpoint = {};
	...
	add(destination) {
		...
		if (CRUD_opertions[sign]) throw new Error("Using CRUD operations in the path is forbidden");
...
class Routes {
	...
	walk(request) {
		...
		if (api) {
			...
			console.log(request, request.socket._httpMessage);
